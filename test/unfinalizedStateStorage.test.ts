...
describe('UnfinalizedStateStorage contract', function () {
  ...
  it('The contract is named "UnfinalizedStateStorage"', async function () {
  ...
  it('Set and get new unfinalized state, expect to get state', async () => {
    ...
    expect(await UnfinalizedStateStorage.getUnfinalizedState(0)).to.be.equal(assertionId);
  });
  ...
  it('Set and delete unfinalized state, expect state to be deleted', async () => {
    ...
    expect(await UnfinalizedStateStorage.getUnfinalizedState(0)).to.be.equal(ZERO_BYTES32);
  });

  it('Set and get new issuer, expect to get issuer', async () => {
    await UnfinalizedStateStorage.setIssuer(0, accounts[0].address);

    expect(await UnfinalizedStateStorage.getIssuer(0)).to.be.equal(accounts[0].address);
  });
  ...
  it('Set and delete issuer, expect issuer to be deleted', async () => {
    ...
    expect(await UnfinalizedStateStorage.getIssuer(0)).to.be.equal(ZERO_ADDRESS);
  });

  it('Set new unfinalized state and check if there is a pending state, expect true', async () => {
    await UnfinalizedStateStorage.setUnfinalizedState(0, assertionId);

    expect(await UnfinalizedStateStorage.hasPendingUpdate(0)).to.be.equal(true);
  });

  it('Check if there is a pending state, expect false', async () => {
    expect(await UnfinalizedStateStorage.hasPendingUpdate(0)).to.be.equal(false);
  });
});
