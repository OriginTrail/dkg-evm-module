...
type TestContracts = {
  ...
  ask: Ask;
  rewardsMigrator: V8_1_1_Rewards_Migrator;
  rewardsMigratorStorage: V8_1_1_Rewards_Migrator_Storage;
};
...
async function setupTestEnvironment(): Promise<{
  ...
  const contracts: Partial<TestContracts> = {
  ...
  const RewardsMigratorStorageFactory = await ethers.getContractFactory(
    'V8_1_1_Rewards_Migrator_Storage',
  );
  contracts.rewardsMigratorStorage =
    (await RewardsMigratorStorageFactory.deploy(
      await contracts.hub.getAddress(),
    )) as V8_1_1_Rewards_Migrator_Storage;
  await contracts.rewardsMigratorStorage.waitForDeployment();
  await contracts.hub
    .connect(accounts.owner)
    .setContractAddress(
      'V8_1_1_Rewards_Migrator_Storage',
      await contracts.rewardsMigratorStorage.getAddress(),
    );

  const RewardsMigratorFactory = await ethers.getContractFactory(
    'V8_1_1_Rewards_Migrator',
  );
  contracts.rewardsMigrator = (await RewardsMigratorFactory.deploy(
    await contracts.hub.getAddress(),
  )) as V8_1_1_Rewards_Migrator;
  await contracts.rewardsMigrator.waitForDeployment();
  await contracts.hub
    .connect(accounts.owner)
    .setContractAddress(
      'V8_1_1_Rewards_Migrator',
      await contracts.rewardsMigrator.getAddress(),
    );

  await contracts.rewardsMigratorStorage.connect(accounts.owner).initialize();
  await contracts.rewardsMigrator.connect(accounts.owner).initialize();
  ...
  return {
    accounts,
    contracts: contracts as TestContracts,
...
describe(`Full complex scenario`, function () {
  ...
  it('Should execute steps 15-23 with detailed score calculations and verification', async function () {
    ...
    /**********************************************************************
     * STEP 18 – Delegator 3 claims rewards for epoch 3
     * --------------------------------------------------------------------
     **********************************************************************/
    ...
    /**********************************************************************
     * STEP 18.5 – MIGRATION TEST: D3 migrates rewards
     **********************************************************************/
    console.log('\n🚚 STEP 18.5: Delegator3 migrates rewards');

    const migratorAmountD3 = toTRAC18(7_000);
    const migratorAmountD1 = toTRAC18(5_000);

    // Set rewards in migrator storage
    console.log('    💰 Setting rewards in migrator storage...');
    await contracts.rewardsMigratorStorage
      .connect(accounts.owner) // onlyHub access
      .setDelegatorsRewards(
        node1Id,
        [accounts.delegator3.address, accounts.delegator1.address],
        [migratorAmountD3, migratorAmountD1],
      );

    const [rewardD3Amount, rewardD3Claimed] =
      await contracts.rewardsMigratorStorage.getReward(
        node1Id,
        accounts.delegator3.address,
      );
    expect(rewardD3Amount).to.equal(migratorAmountD3);
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions
    expect(rewardD3Claimed).to.be.false;
    console.log(
      `    ✅ D3 reward set to ${ethers.formatUnits(migratorAmountD3, 18)} TRAC`,
    );

    // D3 migrates rewards
    console.log('    ▶️  Delegator3 increasing stake base...');
    const d3StakeBaseBeforeMigrate =
      await contracts.stakingStorage.getDelegatorStakeBase(node1Id, d3Key);

    await contracts.rewardsMigrator
      .connect(accounts.delegator3)
      .increaseDelegatorStakeBase(node1Id, accounts.delegator3.address);

    const d3StakeBaseAfterMigrate =
      await contracts.stakingStorage.getDelegatorStakeBase(node1Id, d3Key);

    // Verify stake increase
    expect(d3StakeBaseAfterMigrate).to.equal(
      d3StakeBaseBeforeMigrate + migratorAmountD3,
    );
    console.log(
      `    ✅ D3 stake base increased by ${ethers.formatUnits(migratorAmountD3, 18)} TRAC`,
    );

    // Verify reward is marked as claimed
    const [, rewardD3ClaimedAfter] =
      await contracts.rewardsMigratorStorage.getReward(
        node1Id,
        accounts.delegator3.address,
      );
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions
    expect(rewardD3ClaimedAfter).to.be.true;
    console.log('    ✅ D3 reward marked as claimed in migrator.');
