...
async function ensureNodeHasChunksThisEpoch(
  ...
  console.log(`   🔍 Checking if Node-${nodeId} has chunks this epoch...`);
  const produced =
    await contracts.epochStorage.getNodeCurrentEpochProducedKnowledgeValue(
      nodeId,
    );
  console.log(`   📊 Node-${nodeId} produced value: ${produced}`);

  if (produced === 0n) {
    console.log(`   🏗️  Creating KC to give Node-${nodeId} chunks...`);
    if (
      !receivingNodes.some(
    ...
    console.log(`   ✅ KC created for Node-${nodeId}`);
  } else {
    console.log(`   ✅ Node-${nodeId} already has chunks`);
  }
}
...
async function submitProofAndLogScore(
  ...
  console.log(`\n🔬 ${nodeName} submitting proof for epoch ${epoch}...`);
  ...
  console.log(
    `   📊 ${nodeName} Score: ${scoreBefore} → ${scoreAfter} (gained ${scoreAfter - scoreBefore})`,
  );
  console.log(`   📊 ${nodeName} Score per stake: ${scorePerStake}`);
...
export async function buildInitialRewardsState() {
  ...
  console.log('  D1 → 10k TRAC on Node-1');
  ...
  console.log('  D2 → 20k TRAC on Node-1');
  ...
  console.log('  D3 → 10k TRAC on Node-2');
  ...
  console.log('  D4 → 20k TRAC on Node-2');
  ...
  console.log('  D5 → 30k TRAC on Node-1');
  ...
  console.log('  D6 → 40k TRAC on Node-1');
  ...
  console.log('  D7 → 50k TRAC on Node-1');
  ...
  console.log('  D8 → 30k TRAC on Node-2');
  ...
  console.log('  D9 → 40k TRAC on Node-2');
  ...
  console.log('  D10 → 50k TRAC on Node-2');
  ...
  console.log('  D11 → 60k TRAC on Node-3');
  ...
  console.log('  D12 → 50k TRAC on Node-3');
  ...
  console.log(
    `Current epoch: ${currentEpoch} (epoch-${lastFinalizedEpoch} finalized)\n`,
  );

  console.log('📊 FINAL STAKING DISTRIBUTION:');
  console.log(
    '  EPOCH-2: D1→10k, D2→20k TRAC (Node-1) | D3→10k, D4→20k TRAC (Node-2)',
  );
  console.log(
    '  EPOCH-3: D5→30k, D6→40k, D7→50k TRAC (Node-1) | D8→30k, D9→40k, D10→50k TRAC (Node-2)',
  );
  console.log('           D11→60k, D12→50k TRAC (Node-3)\n');
  ...
  for (const [i, node] of nodes.entries()) {
    ...
    console.log(
      `Node-${i + 1} (id ${node.identityId}) totalStake = ${hre.ethers.formatUnits(
        totalStake,
        18,
      )} TRAC | operatorFee = 10%`,
    );
    console.log(
      `   📊 Epoch-2 Score: ${nodeScore2} | Epoch-3 Score: ${nodeScore3}`,
    );
    ...
    for (let d = 0; d < accounts.delegators.length; d++) {
      ...
      if (stake > 0n) {
        console.log(`   ↳ D${d + 1} ${hre.ethers.formatUnits(stake, 18)} TRAC`);
      }
    }
    console.log('');
  }
