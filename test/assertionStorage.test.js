...
contract('AssertionStorage', async (accounts) => {
  ...
  it('Create an assertion with owner, expect to pass', async () => {
    ...
    await truffleAssert.passes(
      assertionStorage.createAssertion(assertionId, size, triplesNumber, chunksNumber, { from: owner }),
    );
  });

  it('Create an assertion with non owner, expect to fail', async () => {
    await truffleAssert.reverts(
      assertionStorage.createAssertion(assertionId, size, triplesNumber, chunksNumber, { from: nonOwner }),
    );
  });
