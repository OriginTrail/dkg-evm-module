...
// let hub, erc20Token, identity, identityStorage;
...
//     const result = await identity.createIdentity(operational, admin, { from: accounts[0] });
...
//     const result = await identity.createIdentity(accounts[9], accounts[9], { from: accounts[0] });
//     // console.log(result);
//     truffleAssert.eventEmitted(result, 'IdentityCreated');
//   });
...
//     assert(fetchedIdentityId.toString() == 1, 'Failed to get identity');
//   });
...
//     const opKey = ethers.utils.keccak256(ethers.utils.solidityPack(['address'], [accounts[3]]));
//     const adminKey = ethers.utils.keccak256(ethers.utils.solidityPack(['address'], [accounts[4]]));
//     const newAdminKey = ethers.utils.keccak256(ethers.utils.solidityPack(['address'], [accounts[5]]));

//     const result = await identity.createIdentity(accounts[3], accounts[4], { from: accounts[0] });
//     truffleAssert.eventEmitted(result, 'IdentityCreated');
...
//     const resultAddKey = await identity.addKey(identityId, newAdminKey, ADMIN_KEY, ECDSA, { from: accounts[4] });
...
//     const resultRemoveKey = await identity.removeKey(identityId, adminKey, { from: accounts[5] });
