...
contract('Profile', accounts => {
  ...
  before('Deploy a new instance of Profile before tests', async () => {
    ...
    account0Key = ethers.utils.keccak256(
      ethers.utils.solidityPack(['address'], [owner]),
    );
    account1Key = ethers.utils.keccak256(
      ethers.utils.solidityPack(['address'], [accounts[1]]),
    );
  ...
  it('Create a profile with whitelisted node, expect to pass', async () => {
    const receipt = await profile.createProfile(owner, nodeId, 'Token', 'TKN', {
      from: owner,
    });
  ...
  it('Should allow creating a profile (whitelist enabled) for node in the whitelist', async () => {
    ...
    const receipt = await profile.createProfile(
      accounts[1],
      nodeId1,
      'Token1',
      'TKN1',
  ...
  it('Cannot create a profile with existing identity, expect to fail', async () => {
    ...
    await expectRevert(
      profile.createProfile(owner, nodeId, 'Token', 'TKN', { from: owner }),
  ...
  it('Cannot create a profile with registered nodeId, expect to fail', async () => {
    ...
    await expectRevert(
      profile.createProfile(accounts[3], nodeId, 'Token5', 'TKN5', {
        from: accounts[3],
      }),
  ...
  it('Cannot create a profile without tokenName, expect to fail', async () => {
    await whitelistStorage.whitelistAddress(accounts[3]);
    await expectRevert(
      profile.createProfile(accounts[3], nodeId2, '', 'TKN5', {
        from: accounts[3],
      }),
  ...
  it('Cannot create a profile without tokenSymbol, expect to fail', async () => {
    await whitelistStorage.whitelistAddress(accounts[3]);
    await expectRevert(
      profile.createProfile(accounts[3], nodeId2, 'Token5', '', {
        from: accounts[3],
      }),
  ...
  it('Cannot create a profile with taken tokenName, expect to fail', async () => {
    ...
    await whitelistStorage.whitelistAddress(accounts[3]);
    await expectRevert(
      profile.createProfile(accounts[3], nodeId2, 'Token', 'TKN', {
        from: accounts[3],
      }),
  ...
  it('Cannot create a profile with taken tokenSymbol, expect to fail', async () => {
    ...
    await whitelistStorage.whitelistAddress(accounts[3]);
    await expectRevert(
      profile.createProfile(accounts[3], nodeId2, 'Token7', 'TKN', {
        from: accounts[3],
      }),
  ...
  it('Set ask for a profile, expect to pass', async () => {
    const newAsk = 1;
    const setAskResponse = await profile.setAsk(identityId, newAsk, {
      from: owner,
    });
  ...
  it('Stake accumulated operator fee for existing profile, expect to pass', async () => {
    ...
    newIdentityId = (
      await profile.createProfile(owner, nodeId2, 'Token3', 'TKN3', {
        from: accounts[4],
      })
    ).logs[0].args.identityId;
    const getOperatorFee = await profileStorage.getAccumulatedOperatorFee(
    ...
    await expectRevert(
      profile.stakeAccumulatedOperatorFee(newIdentityId.toString(), {
        from: owner,
      }),
      'You have no operator fees',
    );
    await truffleAssert.passes(
      profileStorage.setAccumulatedOperatorFee(
        newIdentityId.toString(),
        newOperatorFeeAmount,
      ),
    );
    const resultStake = await profileStorage.getAccumulatedOperatorFee(
      newIdentityId.toString(),
      { from: owner },
    );
