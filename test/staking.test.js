...
// let erc20Token, hub, profile,
//   // profileStorage, parametersStorage,
//   staking, stakingStorage, identityStorage;
...
//     const stake = await stakingStorage.totalStakes(123);
//     assert(stake).to.equal('456');
//   });
...
//     const opFee = await stakingStorage.operatorFees(123);
//     assert(opFee).to.equal('456');
//   });

//   it('non-Contract should not be able to createWithdrawalRequest; expect to fail', async () => {
//     await truffleAssert.reverts(
//       stakingStorage.createWithdrawalRequest(123, accounts[1], 214, 2022, { from: accounts[9] }),
//     );
//   });
...
//     const itExists = await stakingStorage.withdrawalRequestExists(123, accounts[1]);
//     assert(itExists, 'Withdrawal request does not exist');
//     const amount = await stakingStorage.getWithdrawalRequestAmount(123, accounts[1]);
//     assert(amount).to.equal('214');
//     const timestamp = await stakingStorage.getWithdrawalRequestTimestamp(123, accounts[1]);
//     assert(timestamp).to.equal('2022');
//   });
...
//     const amount = await stakingStorage.getWithdrawalRequestAmount(123, accounts[1]);
//     assert(amount).to.equal('0');
//     const timestamp = await stakingStorage.getWithdrawalRequestTimestamp(123, accounts[1]);
//     assert(timestamp).to.equal('0');
//   });

//   it('Contract should be able to fetch constants; expect to pass', async () => {
//     const name = await stakingStorage.name();
//     assert(name).to.equal('StakingStorage');

//     const version = await stakingStorage.version();
//     assert(version).to.equal('1.0.0');
//   });
...
//     assert(balanceAfterTransfer).to.equal((Number(balanceBeforeTransfer) + 100).toString());
//   });
...
//       accounts[0], ethers.utils.formatBytes32String(peers[0].id), 'Token', 'TKN', { from: accounts[1] },
//     );
...
//     assert(await stakingStorage.totalStakes(identityId).to.equal(stake));
//   });
//   // it('staking contract should not be able to transferStake; expect to fail', async () => {
...
//   //         assert(
//   //           error.message.startsWith(ERROR_PREFIX + 'revert Identity does not exist or user delegation disabled!'),
...
//   //
//   //     assert(
//   //       stakingStorage.totalStakes(nodeIdentityId) == new BN(stake).add(new BN(stake2)).toString(),
//   //       'Total amount of stake is not increased'
//   //     );
//   // });
//   //
...
//   //
//   //     assert(
//   //       stakingStorage.totalStakes(nodeIdentityId) == new BN(stake).add(new BN(reward)).toString(),
