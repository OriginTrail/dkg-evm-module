...
describe('Staking contract', function () {
  ...
  it('finalizeOperatorFeeWithdrawal second call reverts', async () => {
    ...
    expect(
      await StakingStorage.getOperatorFeeWithdrawalRequest(identityId),
    ).to.equal(0n, 'Withdrawal request should be 0');

    await expect(Staking.finalizeOperatorFeeWithdrawal(identityId)).to.be
      .reverted;
  });
  ...
  it('📊 rollingRewards accumulate & auto-restake; earned / paidOut updated', async () => {
    ...
    expect(earned1).to.be.gt(
      tokenRewards1 + tokenRewards2 + tokenRewards3,
      'Earned should be greater than 0',
    );
  ...
  it('📈 Operator-fee percentage path: fee credited once and only once', async () => {
    ...
    // flag must be set
    expect(
      await DelegatorsInfo.getIsOperatorFeeClaimedForEpoch(
        identityId,
        epochNow,
      ),
    ).to.be.true;
    ...
    const leftover = await DelegatorsInfo.getEpochLeftoverDelegatorsRewards(
      identityId,
      epochNow,
    );
  ...
  it('💸 finalizeOperatorFeeWithdrawal happy-path updates cumulatives and transfers tokens', async () => {
    ...
    // Request still records the original amount (contract keeps record)
    const [reqAmtAfter] =
      await StakingStorage.getOperatorFeeWithdrawalRequest(identityId);
    expect(reqAmtAfter).to.equal(
      withdrawAmt,
      'Withdrawal request should keep original amount',
    );
  });
