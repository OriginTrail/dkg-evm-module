...
describe('@unit RandomSamplingStorage', function () {
  ...
  const challengeRecencyFactor =
    parameters.development.RandomSamplingStorage.hardhat.challengeRecencyFactor;
  const bfsIterations =
    parameters.development.RandomSamplingStorage.hardhat.bfsIterations;
  ...
  describe('Scoring System', () => {
    it('Should increment and get epoch node valid proofs count', async () => {
      ...
      await RandomSamplingStorage.connect(
        rsSigner,
      ).incrementEpochNodeValidProofsCount(nextEpoch, nodeId);
    ...
    it('Should add to node score and get node score', async () => {
      ...
      for (let i = 0; i < nodeIds.length; i++) {
        ...
        await RandomSamplingStorage.connect(
          rsSigner,
        ).addToNodeEpochProofPeriodScore(
          currentEpoch,
          proofPeriodIndex,
          nodeId,
          score,
    ...
    it('Should add to and get nodeEpochScorePerStake correctly and emit event', async () => {
      ...
      await expect(
        ...
        .to.emit(RandomSamplingStorage, 'NodeEpochScorePerStakeUpdated')
        .withArgs(nextEpoch, nodeId, scorePerStakeToAdd, scorePerStakeToAdd);
      expect(
        await RandomSamplingStorage.getNodeEpochScorePerStake(
          nextEpoch,
          nodeId,
  ...
  describe('Access Control', () => {
    ...
    it('Should allow access when called by Hub', async () => {
      ...
      await expect(RandomSamplingStorage.connect(hubSigner).initialize()).to.not
        .be.reverted;
  ...
  describe('Proofing Period Management', () => {
    ...
    it('Should pick correct proofing period duration based on epoch', async () => {
      ...
      const newDuration = 1000;
      await RandomSampling.connect(rsSigner).setProofingPeriodDurationInBlocks(
        newDuration,
      );
      ...
      const rsSigner2 = await ethers.getSigner(
        await RandomSampling.getAddress(),
      );
      ...
      await RandomSampling.connect(rsSigner2).setProofingPeriodDurationInBlocks(
        futureDuration,
      );
    ...
    it('Should return correct proofing period duration based on epoch history', async () => {
      ...
      for (let i = 0; i < testEpochs; i++) {
        ...
        await RandomSampling.connect(
          rsSigner,
        ).setProofingPeriodDurationInBlocks(duration);
  ...
  describe('Proofing Period Duration Management', () => {
    it('Should correctly track pending proofing period duration', async () => {
      ...
      await RandomSamplingStorage.connect(
        rsSigner,
      ).replacePendingProofingPeriodDuration(2000, currentEpoch + 1n);
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
      expect(await RandomSamplingStorage.isPendingProofingPeriodDuration()).to
        .be.true;
    ...
    it('Should replace pending duration correctly', async () => {
      ...
      await RandomSamplingStorage.connect(
        rsSigner,
      ).replacePendingProofingPeriodDuration(newDuration, currentEpoch + 1n);
    ...
    it('Should emit ProofingPeriodDurationAdded event with correct parameters', async () => {
      ...
      const tx = await RandomSamplingStorage.connect(
        rsSigner,
      ).addProofingPeriodDuration(newDuration, effectiveEpoch);
    ...
    it('Should emit PendingProofingPeriodDurationReplaced event with correct parameters', async () => {
      ...
      const tx = await RandomSamplingStorage.connect(
        rsSigner,
      ).replacePendingProofingPeriodDuration(newDuration, effectiveEpoch);
      const receipt = await tx.wait();
  ...
  describe('Challenge Recency Factor Management', () => {
    it('Should set correct initial challenge recency factor', async () => {
      // Deploy a new instance to check initial values
      const RandomSamplingStorageFactory = await hre.ethers.getContractFactory(
        'RandomSamplingStorage',
      );
      const challengeRecencyFactor = 5000;
      const bfsIterations = 25;
      const newRandomSamplingStorage =
        await RandomSamplingStorageFactory.deploy(
          Hub.target,
          proofingPeriodDurationInBlocks,
      ...
      expect(
        await newRandomSamplingStorage.getChallengeRecencyFactor(),
      ).to.equal(challengeRecencyFactor);
    });

    it('Should get max challenge recency factor constant', async () => {
      expect(
        await RandomSamplingStorage.getMaxChallengeRecencyFactor(),
      ).to.equal(10000);
    });

    it('Should set challenge recency factor by owner and emit event', async () => {
      const newChallengeRecencyFactor = 7500;
      const oldChallengeRecencyFactor =
        await RandomSamplingStorage.getChallengeRecencyFactor();

      await expect(
        RandomSamplingStorage.connect(accounts[0]).setChallengeRecencyFactor(
          newChallengeRecencyFactor,
        ),
      )
        .to.emit(RandomSamplingStorage, 'ChallengeRecencyFactorUpdated')
        .withArgs(oldChallengeRecencyFactor, newChallengeRecencyFactor);

      expect(await RandomSamplingStorage.getChallengeRecencyFactor()).to.equal(
        newChallengeRecencyFactor,
      );
    });

    it('Should revert if challenge recency factor exceeds maximum', async () => {
      const invalidChallengeRecencyFactor = 10001; // Over the maximum
    ...
    it('Should allow setting challenge recency factor to 0', async () => {
      const zeroChallengeRecencyFactor = 0;
      const oldChallengeRecencyFactor =
        await RandomSamplingStorage.getChallengeRecencyFactor();

      await expect(
        RandomSamplingStorage.connect(accounts[0]).setChallengeRecencyFactor(
          zeroChallengeRecencyFactor,
        ),
      )
        .to.emit(RandomSamplingStorage, 'ChallengeRecencyFactorUpdated')
        .withArgs(oldChallengeRecencyFactor, zeroChallengeRecencyFactor);
    ...
    it('Should allow setting challenge recency factor to maximum', async () => {
      const maxChallengeRecencyFactor = 10000;
      const oldChallengeRecencyFactor =
        await RandomSamplingStorage.getChallengeRecencyFactor();
      ...
      await expect(
        ...
        .to.emit(RandomSamplingStorage, 'ChallengeRecencyFactorUpdated')
        .withArgs(oldChallengeRecencyFactor, maxChallengeRecencyFactor);
    ...
    //   const newChallengeRecencyFactor = 6000;

    //   await expect(
    //     RandomSamplingStorage.connect(accounts[1]).setChallengeRecencyFactor(
    //       newChallengeRecencyFactor,
    //     ),
    //   )
    //     .to.be.revertedWithCustomError(
  ...
  describe('BFS Iterations Management', () => {
    it('Should set correct initial BFS iterations', async () => {
      // Deploy a new instance to check initial values
      const RandomSamplingStorageFactory = await hre.ethers.getContractFactory(
        'RandomSamplingStorage',
      );
      ...
      expect(await newRandomSamplingStorage.getBfsIterations()).to.equal(
        bfsIterations,
      );
    });

    it('Should get default BFS iterations constant', async () => {
      expect(await RandomSamplingStorage.getDefaultBfsIterations()).to.equal(
        50,
      );
    });

    it('Should set BFS iterations by owner and emit event', async () => {
      const newBfsIterations = 75;
      const oldBfsIterations = await RandomSamplingStorage.getBfsIterations();

      await expect(
        RandomSamplingStorage.connect(accounts[0]).setBfsIterations(
          newBfsIterations,
        ),
      )
        .to.emit(RandomSamplingStorage, 'BfsIterationsUpdated')
        .withArgs(oldBfsIterations, newBfsIterations);

      expect(await RandomSamplingStorage.getBfsIterations()).to.equal(
        newBfsIterations,
      );
    });

    it('Should revert if BFS iterations is 0', async () => {
      await expect(
        RandomSamplingStorage.connect(accounts[0]).setBfsIterations(0),
      ).to.be.revertedWith('Iterations must be greater than 0');
    });

    it('Should allow setting BFS iterations to 1', async () => {
      const minBfsIterations = 1;
      const oldBfsIterations = await RandomSamplingStorage.getBfsIterations();
      ...
      await expect(
        ...
        .to.emit(RandomSamplingStorage, 'BfsIterationsUpdated')
        .withArgs(oldBfsIterations, minBfsIterations);
    ...
    it('Should allow setting BFS iterations to maximum uint8 value', async () => {
      const maxBfsIterations = 255; // Maximum uint8 value
      const oldBfsIterations = await RandomSamplingStorage.getBfsIterations();
      ...
      await expect(
        ...
        .to.emit(RandomSamplingStorage, 'BfsIterationsUpdated')
        .withArgs(oldBfsIterations, maxBfsIterations);
    ...
    // // TODO: This test fails because the hub owner is not the multisig owner
    // it('Should revert if called by non-owner', async () => {
    //   const newBfsIterations = 100;

    //   await expect(
    //     RandomSamplingStorage.connect(accounts[1]).setBfsIterations(
    //       newBfsIterations,
    //     ),
    //   )
    //     .to.be.revertedWithCustomError(
  ...
  describe('Constructor Validation for New Parameters', () => {
    it('Should revert if challenge recency factor exceeds maximum in constructor', async () => {
      const RandomSamplingStorageFactory = await hre.ethers.getContractFactory(
        'RandomSamplingStorage',
      );
      const invalidChallengeRecencyFactor = 10001;
      const validBfsIterations = 50;
    ...
    it('Should revert if BFS iterations is 0 in constructor', async () => {
      const RandomSamplingStorageFactory = await hre.ethers.getContractFactory(
        'RandomSamplingStorage',
      );
      const validChallengeRecencyFactor = 5000;
      const invalidBfsIterations = 0;

      await expect(
        RandomSamplingStorageFactory.deploy(
          Hub.target,
          proofingPeriodDurationInBlocks,
          validChallengeRecencyFactor,
          invalidBfsIterations,
        ),
      ).to.be.revertedWith('Bfs iterations must be greater than 0');
    });

    it('Should emit events on constructor with valid parameters', async () => {
      const RandomSamplingStorageFactory = await hre.ethers.getContractFactory(
        'RandomSamplingStorage',
      );
      const challengeRecencyFactor = 6000;
      const bfsIterations = 30;

      const deploymentTx = await RandomSamplingStorageFactory.deploy(
        Hub.target,
        proofingPeriodDurationInBlocks,
        challengeRecencyFactor,
        bfsIterations,
      ...
      // Check for ChallengeRecencyFactorUpdated event
      const challengeRecencyEvent = receipt?.logs.find(
        (log) =>
          log.topics[0] ===
          hre.ethers.id('ChallengeRecencyFactorUpdated(uint16,uint16)'),
      );
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
      ...
      const bfsIterationsEvent = receipt?.logs.find(
        (log) =>
          log.topics[0] === hre.ethers.id('BfsIterationsUpdated(uint8,uint8)'),
      );
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
    ...
    it('Should accept valid edge case values in constructor', async () => {
      ...
      // Test minimum values
      const minChallengeRecencyFactor = 0;
      const minBfsIterations = 1;
      const minInstance = await RandomSamplingStorageFactory.deploy(
        Hub.target,
        proofingPeriodDurationInBlocks,
        minChallengeRecencyFactor,
        minBfsIterations,
      );
      expect(await minInstance.getChallengeRecencyFactor()).to.equal(
        minChallengeRecencyFactor,
      );
      expect(await minInstance.getBfsIterations()).to.equal(minBfsIterations);

      // Test maximum values
      const maxChallengeRecencyFactor = 10000;
      const maxBfsIterations = 255;
      const maxInstance = await RandomSamplingStorageFactory.deploy(
        Hub.target,
        proofingPeriodDurationInBlocks,
        maxChallengeRecencyFactor,
        maxBfsIterations,
      );
      expect(await maxInstance.getChallengeRecencyFactor()).to.equal(
        maxChallengeRecencyFactor,
      );
      expect(await maxInstance.getBfsIterations()).to.equal(maxBfsIterations);
    });
  });

  describe('Multi-Parameter Updates', () => {
    it('Should handle multiple parameter updates correctly', async () => {
      const initialChallengeRecencyFactor =
        await RandomSamplingStorage.getChallengeRecencyFactor();
      const initialBfsIterations =
        await RandomSamplingStorage.getBfsIterations();

      // Update challenge recency factor
      const newChallengeRecencyFactor = 8000;
      await expect(
        RandomSamplingStorage.connect(accounts[0]).setChallengeRecencyFactor(
          newChallengeRecencyFactor,
        ),
        ...
        .to.emit(RandomSamplingStorage, 'ChallengeRecencyFactorUpdated')
        .withArgs(initialChallengeRecencyFactor, newChallengeRecencyFactor);

      // Update BFS iterations
      const newBfsIterations = 100;
      await expect(
        RandomSamplingStorage.connect(accounts[0]).setBfsIterations(
          newBfsIterations,
        ...
        .to.emit(RandomSamplingStorage, 'BfsIterationsUpdated')
        .withArgs(initialBfsIterations, newBfsIterations);
      ...
      // Update challenge recency factor again
      const newerChallengeRecencyFactor = 2500;
      await expect(
        RandomSamplingStorage.connect(accounts[0]).setChallengeRecencyFactor(
          newerChallengeRecencyFactor,
        ),
      )
        .to.emit(RandomSamplingStorage, 'ChallengeRecencyFactorUpdated')
        .withArgs(newChallengeRecencyFactor, newerChallengeRecencyFactor);

      // Verify challenge recency factor is updated but BFS iterations remains unchanged
      expect(await RandomSamplingStorage.getChallengeRecencyFactor()).to.equal(
        newerChallengeRecencyFactor,
      );
      expect(await RandomSamplingStorage.getBfsIterations()).to.equal(
    ...
    it('Should emit correct old values in events after multiple updates', async () => {
      // Set initial values
      const firstChallengeRecencyFactor = 3000;
      const firstBfsIterations = 20;

      await RandomSamplingStorage.connect(
        accounts[0],
      ).setChallengeRecencyFactor(firstChallengeRecencyFactor);
      await RandomSamplingStorage.connect(accounts[0]).setBfsIterations(
        firstBfsIterations,
      );

      // Update and verify events show correct old values
      const secondChallengeRecencyFactor = 9000;
      const secondBfsIterations = 80;

      await expect(
        RandomSamplingStorage.connect(accounts[0]).setChallengeRecencyFactor(
          secondChallengeRecencyFactor,
        ),
      )
        .to.emit(RandomSamplingStorage, 'ChallengeRecencyFactorUpdated')
        .withArgs(firstChallengeRecencyFactor, secondChallengeRecencyFactor);
      ...
      await expect(
        ...
        .to.emit(RandomSamplingStorage, 'BfsIterationsUpdated')
        .withArgs(firstBfsIterations, secondBfsIterations);
    });
  });
});
