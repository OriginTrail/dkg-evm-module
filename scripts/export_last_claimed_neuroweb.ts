...
/* Export lastClaimedEpoch for every delegator on each node (Neuroweb network).
 * Requires RPC_URL_NEURO in .env and uses deployments/neuroweb_mainnet_contracts.json.
 * Output CSV: data/delegators_last_claimed_neuroweb.csv
 */
...
if (!RPC_URL) {
  console.error('RPC_URL_NEURO not set in .env');
  process.exit(1);
}
...
const OUTPUT_CSV = path.join(DATA_DIR, 'delegators_last_claimed_neuroweb.csv');
