...
/* Export lastClaimedEpoch for every delegator on each node (Base network).
 * Requires RPC_URL_BASE in .env and uses deployments/base_mainnet_contracts.json.
 * Output CSV: data/delegators_last_claimed_base.csv with columns:
...
const OUTPUT_CSV = path.join(DATA_DIR, 'delegators_last_claimed_base.csv');
