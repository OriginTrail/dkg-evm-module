...
class Deployer {
  ...
  async validateOTPTransfer (address) {
    ...
    if (addressBalance.toNumber() < INITIAL_TOKEN_AMOUNT) {
      throw Error(
        `Validation failed for otp transfer for address: ${address}. ` +
        `Account balance: ${addressBalance.toNumber()}, required >= ${INITIAL_TOKEN_AMOUNT}`,
      );
    }
  ...
  createSubstrateAddress (evmAddress) {
    ...
    const substrateAddress = execSync(
      `scripts/evm-contract-into-substrate-address ${address}`,
    ).toString().replace(/[\r\n]/gm, '');
