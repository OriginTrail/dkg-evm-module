...
/* Claim delegator rewards for a specific epoch on Gnosis network.
 * Reads RPC_URL_GNOSIS and PRIVATE_KEY from .env.
...
if (!RPC_URL || !PRIVATE_KEY) {
  console.error('RPC_URL_GNOSIS and PRIVATE_KEY must be set in .env');
  process.exit(1);
}
...
const OUTPUT_CSV = path.join(DATA_DIR, 'gnosis_delegators_claim_output.csv');
